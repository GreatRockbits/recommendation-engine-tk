...
def tfidf_recommendations_from_reviews(target_product):
    ...
    other_reviews_with_ids = [(review.review_text, review.product_id) for review in all_reviews.exclude(product_id=target_product) if review.review_text]
    ...
    review_vectorizer.fit(review.review_text for review in all_reviews if review.review_text)
